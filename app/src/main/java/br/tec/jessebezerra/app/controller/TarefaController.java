...
public class TarefaController extends BaseController {
    ...
    @FXML private TableColumn<TarefaTableModel, String> tituloColumn;
    @FXML private TableColumn<TarefaTableModel, Integer> duracaoColumn;
    @FXML private TableColumn<TarefaTableModel, StatusItem> statusColumn;
    @FXML private TableColumn<TarefaTableModel, String> sprintColumn;
    @FXML private TableColumn<TarefaTableModel, String> featureColumn;
    @FXML private TableColumn<TarefaTableModel, String> membroColumn;
    ...
    public void initialize() {
        ...
        tarefaTable.setItems(tarefaList);
        
        tarefaTable.getSelectionModel().selectedItemProperty().addListener(
            (obs, oldSelection, newSelection) -> {
                boolean tarefaSelected = newSelection != null;
                editarButton.setDisable(!tarefaSelected);
                excluirButton.setDisable(!tarefaSelected);
                gerenciarSubsButton.setDisable(!tarefaSelected);
            }
        );
    ...
    protected void onEditarClick() {
        TarefaTableModel selected = tarefaTable.getSelectionModel().getSelectedItem();
        if (selected != null) {
    ...
    protected void onExcluirClick() {
        ...
        if (selected != null) {
            ...
            confirmAlert.setHeaderText("Deseja realmente excluir esta tarefa?");
            confirmAlert.setContentText("Tarefa: " + selected.getTitulo());
            ...
            if (result.isPresent() && result.get() == ButtonType.OK) {
                try {
                    ...
                    showErrorAlert("Erro", "Erro ao excluir tarefa: " + e.getMessage());
                }
            }
        }
    }
    ...
    @FXML
    protected void navigateToHistorias() {
        navigateTo("/br/tec/jessebezerra/app/historia-view.fxml", "Gerenciamento de Histórias");
    }
    
    @FXML
    protected void navigateToTarefas() {
        // Já estamos na tela de tarefas
    }
    ...
    protected void onGerenciarSubsClick() {
        TarefaTableModel selected = tarefaTable.getSelectionModel().getSelectedItem();
        if (selected == null) {
            showErrorAlert("Erro", "Selecione uma tarefa primeiro.");
            return;
        }
    ...
    public static class TarefaTableModel {
    ...
    public void editItem(ItemSprintDTO item) {
        ...
        // Selecionar Feature/História pai
        if (item.getItemPaiId() != null) {
            featureComboBox.getItems().stream()
                .filter(h -> h.getId().equals(item.getItemPaiId()))
                .findFirst()
                .ifPresent(featureComboBox::setValue);
        }
    ...
    protected javafx.stage.Stage getCurrentStage() {
        return (javafx.stage.Stage) tarefaTable.getScene().getWindow();
    }
}
